...
def create_installer():
    ...
    for drive in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
        ...
        if os.path.exists(drive_path):
            inno_paths.extend([
                os.path.join(drive_path, "Program Files (x86)", "Inno Setup 6", "ISCC.exe"),
                os.path.join(drive_path, "Program Files", "Inno Setup 6", "ISCC.exe"),
                os.path.join(drive_path, "Program Files (x86)", "Inno Setup 5", "ISCC.exe"),
                os.path.join(drive_path, "Program Files", "Inno Setup 5", "ISCC.exe")
            ])
    ...
    try:
        ...
        if os.path.exists(installer_path):
            ...
            print("👥 Users can now run HoodieWeatherSetup.exe for guided installation!")
...
def build_release_packages():
    ...
    if success_count > 0:
        ...
        if os.path.exists("../output/installer_output/HoodieWeatherSetup.exe"):
            print("   💾 ../output/installer_output/HoodieWeatherSetup.exe (Windows installer)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--portable-only", action="store_true", 
                       help="Create only portable package")
    parser.add_argument("--installer-only", action="store_true", 
                       help="Create only Windows installer")
    parser.add_argument("--no-build", action="store_true", 
                       help="Skip executable build (use existing)")
